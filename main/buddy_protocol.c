...
int buddy_protocol_parse(const char *json, size_t length, buddy_event_t *event)
{
    ...
    if (strcmp(command, "name") == 0) {
        ...
        result = (int)event->type;
    } else if (strcmp(command, "profile") == 0 || strcmp(command, "focus") == 0 ||
               strcmp(command, "heat") == 0 || strcmp(command, "stamps") == 0 ||
               strcmp(command, "cc") == 0 || strcmp(command, "codex") == 0) {
        result = passport_protocol_parse(root, command, event) ? (int)event->type : result;
