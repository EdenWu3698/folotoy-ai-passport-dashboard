                               ...
                               buddy_action_t *action)
{
    ...
    if (key == BUDDY_KEY_UP) {
        state->page = (buddy_page_t)((state->page + BUDDY_PAGE_COUNT - 1) % BUDDY_PAGE_COUNT);
        buddy_set_ui_refresh(action);
    } else if (key == BUDDY_KEY_DOWN) {
        state->page = (buddy_page_t)((state->page + 1) % BUDDY_PAGE_COUNT);
        buddy_set_ui_refresh(action);
    }
}
                        ...
                        uint64_t now_ms, buddy_action_t *action)
{
    ...
    case BUDDY_EVENT_OWNER:
        ...
        buddy_set_ui_refresh(action);
        break;
    case BUDDY_EVENT_PROFILE:
        state->passport.profile = event->passport.profile;
        buddy_set_ui_refresh(action);
        break;
    case BUDDY_EVENT_FOCUS:
        state->passport.focus = event->passport.focus;
        buddy_set_ui_refresh(action);
        break;
    case BUDDY_EVENT_HEAT:
        state->passport.heat = event->passport.heat;
        buddy_set_ui_refresh(action);
        break;
    case BUDDY_EVENT_STAMPS:
        state->passport.stamps = event->passport.stamps;
        buddy_set_ui_refresh(action);
        break;
    case BUDDY_EVENT_CC:
        state->passport.cc = event->passport.cc;
        state->passport.cc_daily = event->passport.cc_daily;
        buddy_set_ui_refresh(action);
        break;
    case BUDDY_EVENT_CODEX:
        state->passport.codex = event->passport.codex;
        buddy_set_ui_refresh(action);
        break;
...
void buddy_state_snapshot(const buddy_state_t *state, buddy_ui_snapshot_t *snapshot)
{
    ...
    snapshot->passport = state->passport;
