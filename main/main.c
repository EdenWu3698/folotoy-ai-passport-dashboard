...
/* Passport synchronization parses multi-kilobyte JSON and commits a full
 * snapshot in one call chain.  Leave enough headroom for cJSON, NVS and the
 * logging path on the ESP32-C3. */
#define BUDDY_APP_STACK_SIZE 24576U
...
static passport_data_t s_initial_passport;
...
static void buddy_default_name(char name[BUDDY_NAME_MAX])
{
    ...
    if (esp_read_mac(mac, ESP_MAC_BT) == ESP_OK) {
        (void)snprintf(name, BUDDY_NAME_MAX, "Passport-%02X%02X", mac[4], mac[5]);
    } else {
        buddy_copy_text(name, BUDDY_NAME_MAX, "Passport");
    }
}
...
static esp_err_t buddy_factory_reset(buddy_state_t *state)
{
    ...
    if (buddy_settings_factory_reset() != ESP_OK || passport_store_reset() != ESP_OK) {
...
static esp_err_t buddy_orchestrator_commit_passport(void *context,
                                                     const passport_data_t *passport)
{
    (void)context;
    return passport_store_save(passport);
}
...
static buddy_orchestrator_ops_t buddy_orchestrator_ops(buddy_state_t *state)
{
    const buddy_orchestrator_ops_t ops = {
        ...
        .commit_passport = buddy_orchestrator_commit_passport,
...
static void buddy_app_task(void *context)
{
    ...
    state.passport = s_initial_passport;
    buddy_sample_battery(&state);
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "AI Passport Avatar firmware starting");
    ...
    if (buddy_settings_init() != ESP_OK || buddy_settings_load(&s_initial_settings) != ESP_OK ||
        passport_store_init() != ESP_OK || passport_store_load(&s_initial_passport) != ESP_OK) {
