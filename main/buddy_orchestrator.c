                                   ...
                                   uint64_t now_ms, buddy_action_t *action)
{
    ...
    } else if (event.type == BUDDY_EVENT_PROFILE || event.type == BUDDY_EVENT_FOCUS ||
               event.type == BUDDY_EVENT_HEAT || event.type == BUDDY_EVENT_STAMPS ||
               event.type == BUDDY_EVENT_CC || event.type == BUDDY_EVENT_CODEX) {
        static passport_data_t updated;
        updated = state->passport;
        setting_command = true;
        if (event.type == BUDDY_EVENT_PROFILE) updated.profile = event.passport.profile;
        else if (event.type == BUDDY_EVENT_FOCUS) updated.focus = event.passport.focus;
        else if (event.type == BUDDY_EVENT_HEAT) updated.heat = event.passport.heat;
        else if (event.type == BUDDY_EVENT_STAMPS) updated.stamps = event.passport.stamps;
        else if (event.type == BUDDY_EVENT_CC) {
            updated.cc = event.passport.cc;
            updated.cc_daily = event.passport.cc_daily;
        }
        else updated.codex = event.passport.codex;
        if (ops->commit_passport != NULL) {
            err = ops->commit_passport(ops->context, &updated);
        }
    }
