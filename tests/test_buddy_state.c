...
static void test_original_screen_and_menu_navigation(void)
{
    ...
    assert(state.page == BUDDY_PAGE_CODEX);
    buddy_state_reduce(&state, &down, 2, &action);
    assert(state.page == BUDDY_PAGE_PROFILE);
    buddy_state_reduce(&state, &down, 3, &action);
    assert(state.page == BUDDY_PAGE_LINKS);
    buddy_state_reduce(&state, &down, 4, &action);
    assert(state.page == BUDDY_PAGE_FOCUS);
    buddy_state_reduce(&state, &down, 5, &action);
    assert(state.page == BUDDY_PAGE_HEAT);
    buddy_state_reduce(&state, &down, 6, &action);
    assert(state.page == BUDDY_PAGE_STAMPS);
    buddy_state_reduce(&state, &down, 7, &action);
    assert(state.page == BUDDY_PAGE_CC);
    buddy_state_reduce(&state, &down, 8, &action);
    assert(state.page == BUDDY_PAGE_CODEX);

    buddy_state_reduce(&state, &menu, 9, &action);
...
static void test_settings_actions_have_separate_confirmations(void)
{
    ...
    buddy_ui_snapshot_t snapshot;
    buddy_action_t action = {0};
    buddy_event_t profile = {.type = BUDDY_EVENT_PROFILE};
    buddy_event_t cc = {.type = BUDDY_EVENT_CC};

    snprintf(profile.passport.profile.name,
             sizeof(profile.passport.profile.name), "%s", "TRAVELER");
    profile.passport.profile.valid = true;
    buddy_state_init(&state, NULL);
    buddy_state_reduce(&state, &profile, 1000, &action);
    assert(action.type == BUDDY_ACTION_UI_REFRESH);
    buddy_state_snapshot(&state, &snapshot);
    assert(snapshot.passport.profile.valid);
    assert(strcmp(snapshot.passport.profile.name, "TRAVELER") == 0);
    cc.passport.cc.valid = true;
    cc.passport.cc.five_hour_used = 42;
    cc.passport.cc_daily.valid = true;
    cc.passport.cc_daily.tokens = 1200;
    buddy_state_reduce(&state, &cc, 1001, &action);
    buddy_state_snapshot(&state, &snapshot);
    assert(snapshot.passport.cc.valid);
    assert(snapshot.passport.cc.five_hour_used == 42);
    assert(snapshot.passport.cc_daily.tokens == 1200);
}
...
static void test_original_settings_surface_is_complete_and_bounded(void)
{
    ...
    assert(BUDDY_PAGE_COUNT == 7);
    buddy_state_init(&state, NULL);
    state.page = BUDDY_PAGE_CODEX;
    buddy_state_reduce(&state, &down, 1, &action);
    assert(state.page == BUDDY_PAGE_PROFILE);
}
