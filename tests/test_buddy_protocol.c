...
static void test_passport_sync_commands(void)
{
    buddy_event_t event = {0};
    ...
    assert(parse("{\"cmd\":\"profile\",\"name\":\"TRAVELER\","
                 "\"tagline\":\"EXPLORING WITH AI\","
                 "\"into\":[\"AI\",\"CODE\",\"HARDWARE\"],"
                 "\"links\":[{\"l\":\"GITHUB\",\"u\":\"https://github.com/your-name\"}],"
                 "\"rev\":\"A\",\"no\":\"0001\",\"issued\":\"2026-09-05\"}",
                 &event) == BUDDY_EVENT_PROFILE);
    assert(event.passport.profile.valid);
    assert(strcmp(event.passport.profile.name, "TRAVELER") == 0);
    assert(event.passport.profile.link_count == 1);

    assert(parse("{\"cmd\":\"focus\",\"items\":[{\"t\":\"CODE\",\"c\":12,\"d\":7}]}",
                 &event) == BUDDY_EVENT_FOCUS);
    assert(event.passport.focus.count == 1);
    assert(event.passport.focus.items[0].sessions == 12);

    for (index = 0; index < PASSPORT_HEAT_DAYS; ++index) {
        levels[index] = (char)('0' + index % 5U);
    }
    levels[PASSPORT_HEAT_DAYS] = '\0';
    snprintf(heat_json, sizeof(heat_json),
             "{\"cmd\":\"heat\",\"start\":\"2026-06-07\",\"all\":\"%s\","
             "\"themes\":{\"CODE\":\"%s\"},"
             "\"sum\":{\"days\":20,\"tok\":1234,\"streak\":4,"
             "\"week\":300,\"today\":100,\"total\":1234}}",
             levels, levels);
    assert(parse(heat_json, &event) == BUDDY_EVENT_HEAT);
    assert(event.passport.heat.valid);
    assert(event.passport.heat.summary.tokens == 1234);

    assert(parse("{\"cmd\":\"stamps\",\"items\":[{\"t\":\"AI\",\"d\":\"2026-09-05\"}]}",
                 &event) == BUDDY_EVENT_STAMPS);
    assert(event.passport.stamps.count == 1);

    assert(parse("{\"cmd\":\"cc\",\"provider\":\"Kimi For Coding\","
                 "\"five\":{\"used\":6,\"reset\":1788613261},"
                 "\"week\":{\"used\":98,\"reset\":1788937261},"
                 "\"today\":{\"tok\":54300324,\"req\":434,\"out\":206965},"
                 "\"updated\":1788605000,\"ok\":1}", &event) == BUDDY_EVENT_CC);
    assert(event.passport.cc.valid);
    assert(event.passport.cc.five_hour_used == 6);
    assert(event.passport.cc.week_used == 98);
    assert(event.passport.cc.week_resets_at == 1788937261);
    assert(event.passport.cc_daily.tokens == 54300324);
    assert(event.passport.cc_daily.requests == 434);

    assert(parse("{\"cmd\":\"codex\","
                 "\"main\":{\"used\":64,\"mins\":10080,\"reset\":1788761028},"
                 "\"spark\":{\"used\":3,\"reset\":1788623387,"
                 "\"week_used\":7,\"week_reset\":1789210187},"
                 "\"credits\":3,\"updated\":1788605000,\"ok\":1}",
                 &event) == BUDDY_EVENT_CODEX);
    assert(event.passport.codex.valid);
    assert(event.passport.codex.main_used == 64);
    assert(event.passport.codex.spark_5h_used == 3);
    assert(event.passport.codex.reset_credits == 3);

    assert(parse("{\"cmd\":\"heat\",\"start\":\"2026-06-07\",\"all\":\"123\","
                 "\"themes\":{},\"sum\":{\"days\":0,\"tok\":0,\"streak\":0,"
                 "\"week\":0,\"today\":0,\"total\":0}}",
                 &event) == BUDDY_EVENT_MALFORMED);
}
